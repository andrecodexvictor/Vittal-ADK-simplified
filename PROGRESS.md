...
## Log
- 2026-07-24: **Hardening do financeiro (pré-homologação 06/08)**: (1) LGPD — `mapInvoice` agora lê a `placa` do boleto (a validação CPF↔placa era no-op com a forma real da resposta Hinova); tool nega quando o vínculo é inverificável e expõe só as faturas da placa validada (multi-veículo não vaza). (2) Régua — ledger persistente (`.billing-cache/ledger.json`): máx. 1 msg por boleto+estágio, "vencido" repete a cada 7 dias (antes: lembrete diário por semanas). (3) `BILLING_MAX_SENDS_PER_RUN=100` — teto anti-ban por execução (dry-run de hoje: **742 elegíveis** — sem teto, o 1º dia live estouraria o número novo). 8 testes novos (21 no financeiro, 38 no maestro). Core replicado nos 2 repos.
