...
export class SgaClient {
  ...
  async getFinancialInvoice(input: { cpf: string; plate: string }): Promise<{ invoices: SgaInvoice[] }> {
    // A rota exige janela de vencimento (dd/mm/yyyy) e limita o intervalo a 365 dias:
    // 300 dias atrás → 60 dias à frente (360d) cobre vencidos e a vencer do associado.
    const toBr = (d: Date) =>
    ...
    const data = await this.request<any>({
      ...
      body: {
        ...
        data_vencimento_inicial: toBr(new Date(now.getTime() - 300 * 24 * 3600_000)),
